...
export const UNIFOR_COURSE_NAMES = UNIFOR_COURSES.map((c) => c.name);

export function isUniforCourse(name: string): boolean {
  return UNIFOR_COURSE_NAMES.includes(name.trim());
}
...
export function coursesByArea(): Array<{
  area: CourseAreaId;
  label: string;
  courses: UniforCourse[];
}> {
  return (Object.keys(COURSE_AREAS) as CourseAreaId[]).map((area) => ({
    area,
    label: COURSE_AREAS[area],
    courses: UNIFOR_COURSES.filter((c) => c.area === area),
  }));
}
